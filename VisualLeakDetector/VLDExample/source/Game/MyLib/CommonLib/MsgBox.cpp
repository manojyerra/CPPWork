...
MsgBox::MsgBox(string msg)
{
	...
	_bg = new ButtonNew("", (ow-bgW)/2, (oh-bgH)/2, bgW, bgH, ButtonNew::COLOR1);
...
{
	ButtonNew* btn = new ButtonNew(text, 0,_bg->GetY()+_bg->GetHeight()-1.5*_btnH,_btnW, _btnH, ButtonNew::COLOR_EMPTY);
...
void MsgBox::Draw()
{
	if(_isActive)
	{
		...
		RectPrim::Draw(0,0,ow,oh, 247,100,236,228, 253,198,98,228);

		_bg->Draw();

		int bgX = _bg->GetX();
		int bgY = _bg->GetY();
		int bgW = _bg->GetWidth();
		int bgH = _bg->GetHeight();
		...
		float startX = _bg->GetX() + _bg->GetWidth()/2 - totW/2;
