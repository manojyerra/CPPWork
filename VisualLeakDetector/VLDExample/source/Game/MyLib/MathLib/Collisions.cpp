...
bool Collisions::IsColliding(Circle* circle, Point* p1, Point* p2)
{
	...
	else
	{
        ...
        Point footPoint = Line2D::FootOfPerpendicular(p1, p2, circle->GetOrigin());
		return (lineSeg.Contains(&footPoint));
	}
...
bool Collisions::IsColliding(Circle* circle, Rect2D* rect)
{
    ...
    Point rectP1 = rect->GetVertex1();
    Point rectP2 = rect->GetVertex1();
    Point rectP3 = rect->GetVertex1();
    Point rectP4 = rect->GetVertex1();
...
{
	Point p = Plane::IntersectionPoint_LineSegment3D(&triangle3D->P1, &triangle3D->P2, &triangle3D->P3, &line->P1, &line->P2);
